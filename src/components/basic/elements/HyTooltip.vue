<template>
  <div class="hy-tooltip-target" ref="tooltipTarget">
    ...
    <Teleport to="#teleport-area" v-if="!!messageCode">
      <div
        class="hy-tooltip-container"
        ref="tooltipContainer"
        :style="{
          top: toPx(tooltipCoordinates.top),
          right: toPx(tooltipCoordinates.right),
          bottom: toPx(tooltipCoordinates.bottom),
          left: toPx(tooltipCoordinates.left),
          opacity: tooltipStyles.opacity,
          '--text-color': tooltipStyles.textColor,
          '--border-color': tooltipStyles.borderColor,
        }"
      >
        {{ resolveMessage() }}
      </div>
    ...
    calculateTooltipCoordinates(tooltipTargetRef, tooltipTargetCoordinates, stickTo) {
      ...
      if (stickTo === 'top-right') {
        if (tooltipTargetCoordinates.bottom / viewportHeight > 0.8) {
          return this.calculateTooltipCoordinates(
            tooltipTargetRef,
            tooltipTargetCoordinates,
            'bottom-right',
          )
