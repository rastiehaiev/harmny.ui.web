  ...
  <div class="hy-menu__container">
    ...
    <Teleport to="#teleport-area" v-if="menuDisplay">
      <OnClickOutside @trigger="closeMenu" :options="{ ignore: [this.$refs.actuator] }">
        <div
          class="hy-menu__menu-area"
          ref="menuArea"
          @click="closeMenu"
          :style="{
            top: toPx(menuCoordTop),
            right: toPx(menuCoordRight),
            bottom: toPx(menuCoordBottom),
            left: toPx(menuCoordLeft),
          }"
        >
    ...
    toggleMenu() {
      ...
      if (this.menuDisplay) {
        this.$nextTick(() => {
          ...
          if (
            actuatorCoordinates.x !== this.actuatorCoordX ||
            actuatorCoordinates.y !== this.actuatorCoordY
          ) {
            ...
            const { top, right, bottom, left } = this.calculateMenuCoordinates(
              actuatorCoordinates,
              menuAreaCoordinates,
            )
